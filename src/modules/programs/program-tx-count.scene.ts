...
interface WizardSessionData {
    cursor: number;
    current: string;
    state: ProgramTxCountWizardState;
}
...
export class ProgramTxCountScene {
    ...
    @WizardStep(1)
    async askProgramAddress(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${PROGRAM_TX_COUNT_SCENE_ID}, step 1: askProgramAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: PROGRAM_TX_COUNT_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async askRange(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_TX_COUNT_SCENE_ID}, step 2: askRange, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
    ...
    @WizardStep(3)
    async handleTxCountQuery(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_TX_COUNT_SCENE_ID}, step 3: handleTxCountQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            if (!txCounts || txCounts.length === 0) {
                this.logger.debug(`No transaction counts found for programAddress: ${programAddress}, range: ${range}`);
                await ctx.replyWithHTML(
            ...
            this.logger.error(`Error in handleTxCountQuery: ${error.message}, stack: ${error.stack}`);
            await handleErrorResponse({
                ctx,
                error,
                defaultMessage: BOT_MESSAGES.ERROR.API_ERROR,
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.PROGRAM_TX_COUNT_AGAIN:
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }, command: string) {
    ...
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: ProgramTxCountWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
