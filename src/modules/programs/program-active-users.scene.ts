...
interface WizardSessionData {
    cursor: number;
    current: string;
    state: ProgramActiveUsersWizardState;
}
...
export class ProgramActiveUsersScene {
    ...
    @WizardStep(1)
    async askProgramAddress(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${PROGRAM_ACTIVE_USERS_SCENE_ID}, step 1: askProgramAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: PROGRAM_ACTIVE_USERS_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async askDays(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_ACTIVE_USERS_SCENE_ID}, step 2: askDays, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);

            // Handle callback queries
            if (ctx.updateType === 'callback_query') {
                const data = (ctx.callbackQuery as any).data;
                this.logger.debug(`Callback query received: ${data}, handling in askDays`);
            ...
            ctx.wizard.next();
            ctx.session.__scenes = ctx.session.__scenes || { cursor: 0, current: '', state: {} };
            ...
            this.logger.error(`Error in askDays: ${error.message}, stack: ${error.stack}`);
            await ctx.scene.leave();
            ctx.session = {};
        }
    }
    ...
    @WizardStep(3)
    async askSort(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_ACTIVE_USERS_SCENE_ID}, step 3: askSort, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            if (ctx.updateType === 'callback_query') {
                ...
                const days = parseInt(messageText, 10);
                if (!days || days < 1 || days > 30) {
                    this.logger.debug(`Invalid days input: ${messageText}`);
                    await ctx.replyWithHTML(
                        BOT_MESSAGES.ERROR.INVALID_DAYS,
                        { reply_markup: this.keyboard.getProgramActiveUsersKeyboard().reply_markup }
                    );
            ...
            this.logger.error(`Error in askSort: ${error.message}, stack: ${error.stack}`);
            await ctx.scene.leave();
            ctx.session = {};
        }
    }
    ...
    @WizardStep(4)
    async handleActiveUsersQuery(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_ACTIVE_USERS_SCENE_ID}, step 4: handleActiveUsersQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            if (ctx.updateType === 'callback_query') {
                ...
                if (data.startsWith('sort:')) {
                    const [_, field, order] = data.split(':');
            ...
            if (!activeUsers || activeUsers.length === 0) {
                this.logger.debug(`No active users found for programAddress: ${programAddress}, days: ${days}`);
                await ctx.replyWithHTML(
            ...
            this.logger.error(`Error in handleActiveUsersQuery: ${error.message}, stack: ${error.stack}`);
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.PROGRAM_ACTIVE_USERS_AGAIN:
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }, command: string) {
    ...
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
