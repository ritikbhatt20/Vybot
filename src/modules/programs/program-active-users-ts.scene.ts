...
interface WizardSessionData {
    cursor: number;
    current: string;
    state: ProgramActiveUsersTsWizardState;
}
...
export class ProgramActiveUsersTsScene {
    ...
    @WizardStep(1)
    async askProgramAddress(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${PROGRAM_ACTIVE_USERS_TS_SCENE_ID}, step 1: askProgramAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: PROGRAM_ACTIVE_USERS_TS_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async askRange(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_ACTIVE_USERS_TS_SCENE_ID}, step 2: askRange, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
    ...
    @WizardStep(3)
    async handleActiveUsersQuery(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PROGRAM_ACTIVE_USERS_TS_SCENE_ID}, step 3: handleActiveUsersQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            if (!activeUsers || activeUsers.length === 0) {
                this.logger.debug(`No active users found for programAddress: ${programAddress}, range: ${range}`);
                await ctx.replyWithHTML(
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.PROGRAM_ACTIVE_USERS_TS_AGAIN:
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }, command: string) {
    ...
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: ProgramActiveUsersTsWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
