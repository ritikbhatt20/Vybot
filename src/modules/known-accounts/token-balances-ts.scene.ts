...
interface TokenBalancesTsWizardState {
    ownerAddress?: string;
    days?: number;
}
...
interface WizardSessionData {
    cursor: number;
    current: string;
    state: TokenBalancesTsWizardState;
}
...
export class TokenBalancesTsScene {
    ...
    @WizardStep(1)
    async askAddress(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${TOKEN_BALANCES_TS_SCENE_ID}, step 1: askAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: TOKEN_BALANCES_TS_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async askDays(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_BALANCES_TS_SCENE_ID}, step 2: askDays, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            ctx.wizard.next();
            ctx.session.__scenes = ctx.session.__scenes ?? { cursor: 0, current: TOKEN_BALANCES_TS_SCENE_ID, state: {} };
    ...
    @WizardStep(3)
    async handleBalancesQuery(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_BALANCES_TS_SCENE_ID}, step 3: handleBalancesQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);

            // Handle callback queries (e.g., days selection)
            if (ctx.updateType === 'callback_query') {
                const data = (ctx.callbackQuery as any).data;
                if ([SceneActions.CANCEL_BUTTON, SceneActions.MAIN_MENU_BUTTON].includes(data)) {
                    this.logger.debug(`Skipping callback ${data} in handleBalancesQuery, handled by @Action`);
                    return;
                }
            }
            ...
            if (!Array.isArray(balances) || balances.length === 0) {
                this.logger.debug(`No token balances found for address: ${ownerAddress}, days: ${queryDays}`);
                await ctx.replyWithHTML(
            ...
            this.logger.error(`Error in handleBalancesQuery: ${error.message}, stack: ${error.stack}`);
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.TOKEN_BALANCES_TS_AGAIN:
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }, command: string) {
    ...
    @Action(/days:\d+/)
    async handleDaysSelection(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        await this.handleBalancesQuery(ctx);
    }

    @Action(SceneActions.TOKEN_BALANCES_TS_AGAIN)
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesTsWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
