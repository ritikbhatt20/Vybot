...
interface TokenBalancesWizardState {
    address?: string;
}
...
interface WizardSessionData {
    cursor: number;
    current: string;
    state: TokenBalancesWizardState;
}
...
export class TokenBalancesScene {
    ...
    @WizardStep(1)
    async askAddress(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${TOKEN_BALANCES_SCENE_ID}, step 1: askAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: TOKEN_BALANCES_SCENE_ID,
                state: {}
            };
            ...
            ctx.wizard.next();
            ctx.session.__scenes.cursor = 1; // Manually set cursor to avoid NaN
    ...
    @WizardStep(2)
    async handleAddress(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_BALANCES_SCENE_ID}, step 2: handleAddress, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);

            // Handle callback queries
            if (ctx.updateType === 'callback_query') {
                const data = (ctx.callbackQuery as any).data;
                this.logger.debug(`Callback query received: ${data}, handling in handleAddress`);
            ...
            if (!messageText || !/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(messageText)) {
                this.logger.debug(`Invalid or no message text received, prompting for valid input`);
                await ctx.replyWithHTML(
            ...
            this.logger.debug(`Processing address: ${messageText}`);
            ...
            this.logger.debug(`API Response received: ${JSON.stringify(response)}`);

            if (!response.data || response.data.length === 0) {
                this.logger.debug(`No token balances found for address: ${messageText}`);
                await ctx.replyWithHTML(
            ...
            this.logger.error(`Error in handleAddress: ${error.message}, stack: ${error.stack}`);
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.TOKEN_BALANCES_AGAIN:
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }, command: string) {
    ...
    async onTokenBalances(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Token Balances button clicked, entering ${TOKEN_BALANCES_SCENE_ID}`);
            await ctx.answerCbQuery();
            await ctx.scene.enter(TOKEN_BALANCES_SCENE_ID);
        } catch (error) {
            this.logger.error(`Error in onTokenBalances: ${error.message}`);
            await ctx.replyWithHTML(BOT_MESSAGES.ERROR.GENERIC);
        }
    }

    @Action(SceneActions.TOKEN_BALANCES_AGAIN)
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: TokenBalancesWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
