...
export class TokenPriceScene {
    ...
    @WizardStep(1)
    async askMintAddress(@Ctx() ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${TOKEN_PRICE_SCENE_ID}, step 1: askMintAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: TOKEN_PRICE_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async handlePriceQuery(@Ctx() ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_PRICE_SCENE_ID}, step 2: handlePriceQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            if (!tokenPriceData) {
                this.logger.debug(`No token price data found for mintAddress: ${mintAddress}`);
                await ctx.replyWithHTML(
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.TOKEN_PRICE_AGAIN:
                    await ctx.replyWithHTML(BOT_MESSAGES.PRICES.TOKEN_PRICE.ASK_MINT_ADDRESS, {
                        reply_markup: this.keyboard.getTokenPriceKeyboard().reply_markup,
                    });
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }, command: string) {
    ...
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: TokenPriceWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
