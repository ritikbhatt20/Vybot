...
export class PythProductScene {
    ...
    @WizardStep(1)
    async askProductId(@Ctx() ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${PYTH_PRODUCT_SCENE_ID}, step 1: askProductId, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: PYTH_PRODUCT_SCENE_ID,
                state: {}
            };
            ...
            this.logger.error(`Error in askProductId: ${error.message}`);
    ...
    @WizardStep(2)
    async handleProductQuery(@Ctx() ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PYTH_PRODUCT_SCENE_ID}, step 2: handleProductQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);

            // Handle callback queries
            if (ctx.updateType === 'callback_query') {
                const data = (ctx.callbackQuery as any).data;
                this.logger.debug(`Callback query received: ${data}, handling in handleProductQuery`);
            ...
            if ('code' in product && 'message' in product) {
                this.logger.debug(`API error: ${product.message}`);
                await ctx.replyWithHTML(
            ...
            this.logger.error(`Error in handleProductQuery: ${error.message}, stack: ${error.stack}`);
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.PYTH_PRODUCT_AGAIN:
                    await ctx.replyWithHTML(BOT_MESSAGES.PRICES.PYTH_PRODUCT.ASK_PRODUCT_ID, {
                        reply_markup: this.keyboard.getPythProductResultsKeyboard().reply_markup,
                    });
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }, command: string) {
    ...
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: PythProductWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
