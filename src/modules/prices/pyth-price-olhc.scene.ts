...
interface WizardSessionData {
    cursor: number;
    current: string;
    state: PythPriceOhlcWizardState;
}
...
export class PythPriceOhlcScene {
    ...
    @WizardStep(1)
    async askPriceFeedId(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${PYTH_PRICE_OHLC_SCENE_ID}, step 1: askPriceFeedId, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: PYTH_PRICE_OHLC_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async askStartTime(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PYTH_PRICE_OHLC_SCENE_ID}, step 2: askStartTime, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            ctx.wizard.next();
            ctx.session.__scenes = ctx.session.__scenes || { cursor: 0, current: PYTH_PRICE_OHLC_SCENE_ID, state: {} };
    ...
    @WizardStep(3)
    async askEndTime(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PYTH_PRICE_OHLC_SCENE_ID}, step 3: askEndTime, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
    ...
    @WizardStep(4)
    async askResolution(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PYTH_PRICE_OHLC_SCENE_ID}, step 4: askResolution, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
    ...
    @WizardStep(5)
    async handleOhlcQuery(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${PYTH_PRICE_OHLC_SCENE_ID}, step 5: handleOhlcQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);

            // Handle callback queries
            if (ctx.updateType === 'callback_query') {
                const data = (ctx.callbackQuery as any).data;
                this.logger.debug(`Callback query received: ${data}, handling in handleOhlcQuery`);
                await ctx.answerCbQuery();
            ...
            if (!ohlcData || ohlcData.length === 0) {
                this.logger.debug(`No OHLC data found for priceFeedId: ${priceFeedId}, timeStart: ${timeStart}, timeEnd: ${timeEnd}, resolution: ${resolution}`);
                await ctx.replyWithHTML(
                    BOT_MESSAGES.PRICES.PYTH_PRICE_OHLC.NO_RESULTS,
                    { reply_markup: this.keyboard.getPythPriceOhlcResultsKeyboard().reply_markup }
                );
            ...
            this.logger.error(`Error in handleOhlcQuery: ${error.message}, stack: ${error.stack}`);
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }, data: string) {
        this.logger.debug(`Handling callback after scene exit: ${data}`);

        try {
            switch (data) {
                case SceneActions.PYTH_PRICE_OHLC_AGAIN:
                    await ctx.replyWithHTML(BOT_MESSAGES.PRICES.PYTH_PRICE_OHLC.ASK_PRICE_FEED_ID, {
                        reply_markup: this.keyboard.getPythPriceOhlcResultsKeyboard().reply_markup,
                    });
    ...
    private async handleCommand(ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }, command: string) {
    ...
    async tryAgain(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
    ...
    async onCancel(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
    ...
    async onMainMenu(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
    ...
    async cancelCommand(@Ctx() ctx: WizardContext & { wizard: { state: PythPriceOhlcWizardState }, session: CustomSession }) {
        try {
            await ctx.replyWithHTML(BOT_MESSAGES.CANCEL, {
                reply_markup: this.keyboard.getMainKeyboard().reply_markup,
            });
