...
export class TokenHoldersTimeSeriesScene {
    ...
    async askMintAddress(@Ctx() ctx: WizardContext & { wizard: { state: TokenHoldersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Entering ${TOKEN_HOLDERS_TS_SCENE_ID}, step 1: askMintAddress, session: ${JSON.stringify(ctx.session)}`);

            // Initialize session if undefined
            ctx.session = ctx.session || {};
            ctx.session.__scenes = ctx.session.__scenes || {
                cursor: 0,
                current: TOKEN_HOLDERS_TS_SCENE_ID,
                state: {}
            };
    ...
    @WizardStep(2)
    async askStartTime(@Ctx() ctx: WizardContext & { wizard: { state: TokenHoldersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_HOLDERS_TS_SCENE_ID}, step 2: askStartTime, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
            ...
            ctx.wizard.next();
            ctx.session.__scenes = ctx.session.__scenes || { cursor: 0, current: TOKEN_HOLDERS_TS_SCENE_ID, state: {} };
    ...
    @WizardStep(3)
    async askEndTime(@Ctx() ctx: WizardContext & { wizard: { state: TokenHoldersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_HOLDERS_TS_SCENE_ID}, step 3: askEndTime, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);
    ...
    @WizardStep(4)
    async handleHoldersQuery(@Ctx() ctx: WizardContext & { wizard: { state: TokenHoldersWizardState }, session: CustomSession }) {
        try {
            this.logger.debug(`Processing ${TOKEN_HOLDERS_TS_SCENE_ID}, step 4: handleHoldersQuery, updateType: ${ctx.updateType}, scene: ${ctx.scene.current?.id}, cursor: ${ctx.wizard.cursor}, session: ${JSON.stringify(ctx.session)}`);

            // Handle callback queries
            if (ctx.updateType === 'callback_query') {
                const data = (ctx.callbackQuery as any).data;
                this.logger.debug(`Callback query received: ${data}, handling in handleHoldersQuery`);
                await ctx.answerCbQuery();
                await ctx.scene.leave();
                await this.handleCallback(ctx, data);
                return;
            ...
            const { mintAddress, startTime } = ctx.wizard.state;
            ...
            if (!holders || holders.length === 0) {
                this.logger.debug(`No holders found for mintAddress: ${mintAddress}, startTime: ${startTime}, endTime: ${endTime}`);
                await ctx.replyWithHTML(
            ...
            await ctx.replyWithHTML(
                `${BOT_MESSAGES.TOKEN_HOLDERS_TS.RESULTS_HEADER.replace('{token}', `${tokenName} (${tokenSymbol})`)}${message}`,
                { reply_markup: this.keyboard.getTokenHoldersTsResultsKeyboard().reply_markup }
            );
            ...
            this.logger.error(`Error in handleHoldersQuery: ${error.message}, stack: ${error.stack}`);
    ...
    private async handleCallback(ctx: WizardContext & { wizard: { state: TokenHoldersWizardState }, session: CustomSession }, data: string) {
        ...
        try {
            switch (data) {
                case SceneActions.TOKEN_HOLDERS_TS_AGAIN:
